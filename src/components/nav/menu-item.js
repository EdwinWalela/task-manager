...
function MenuItem(props){
  ...
  return (
    <div className={`flex py-3 px-6 mb-2 cursor-pointer group transition-all duration-200 ease-out hover:border-l-4 border-purple-700 ${props.active ? "border-l-4 border-purple-700":""}`}>
      ...
      <p className={`text-gray-400 font-medium flex-1 ${!props.active ? "hover:scale-95 ":""} transition-all duration-200 ease-out group-hover:text-purple-700 ${props.active ? "text-purple-700":""}`}>{props.title}</p>
